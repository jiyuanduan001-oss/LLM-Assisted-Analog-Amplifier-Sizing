...
| `C_int_LOAD` | — | `Cgs_cas + Cdb5 + Cgd5` (+ Csb_cas ≈ Cdb_cas for PFET cas with bulk at VDD) |
...
* Cascode and lv_cascode share the same `C_mirror` — they differ only in
...
* The reference-side transconductance setting the pole is still `gm6`
  ...
  the effective diode conductance `≈ gm6` to first order.
...
sub-block:
`PM = 90° - arctan(GBW/fp2) + arctan(GBW/fz2) - arctan(GBW/p_int_LOAD)`

Constraint for 60° PM with cascode: `p_int_LOAD > 3 × 2π·GBW`.
...
|------|----------------|-------------|
| Output (vout) | M1 drain, M5 drain | `C_out = CL + Cgd1 + Cdb1 + C_eq_LOAD` — for single load `C_eq_LOAD = Cgd5 + Cdb5` |
| Mirror (net1) | M2 drain, M6 drain/gate, M5 gate | `C_mirror = Cgs5 + Cgs6 + Cdb6 + Cgd2 + Cdb2` |
| Cascode internal (net_out5 / net_int_m) | M5/M6 drain, cas source | `C_int_LOAD = Cgs_cas + Cdb5 + Cgd5` (+ Csb_cas ≈ Cdb_cas if PFET cas source ≠ VDD) |
